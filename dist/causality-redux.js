...
var invalidChangerKeys = ['setState', 'getState', 'subscribe', 'partitionState'];
...
var pluginRequiredKeys = ['pluginId'];
...
var indicateStateChange = function indicateStateChange(partitionName, type, operation, prevState, nextState, changerName, theirArgs) {
    if (typeof _onStateChange === 'function') {
        if (changerName === 'setState') operation = 'setState';
        var obj = {
...
var internalPartitionSetState = function internalPartitionSetState(store, partitionName) {
    return function (theArg) {
        if (!objectType(theArg)) error('The Argument to setState must be an object.');
        var actionObj = {};
...
var buildStateEntryChangersAndReducers = function buildStateEntryChangersAndReducers(entry) {
    ...
    var isArrayOperation = function isArrayOperation(arg) {
        return arg === operations.STATE_ARRAY_ADD || arg === operations.STATE_ARRAY_DELETE || arg === operations.STATE_ARRAY_ENTRY_MERGE;
    };
    ...
    var buildChanger = function buildChanger(partitionName, changerName, changerArg) {
        return function () {
            ...
            actionObj.operation = _typeof(changerArg.operation) === undefinedString ? operations.STATE_COPY : changerArg.operation;
            actionObj.partitionName = partitionName;
            ...
            if (isArrayOperation(changerArg.operation)) {
                ...
                if (changerArg.operation === operations.STATE_ARRAY_DELETE || changerArg.operation === operations.STATE_ARRAY_ENTRY_MERGE) actionObj.arrayArg = theArgs[0].toString();else actionObj.arrayArg = theArgs[0];
    ...
    var internalDefinedReducer = function internalDefinedReducer(changerName, changerArg) {
        return function (state, action) {
            ...
            var key = void 0,
                index = void 0;
            if (_typeof(action.operation) === undefinedString) action.operation = operations.STATE_COPY;
...
function init(partitionDefinitions, preloadedState, enhancer) {
    ...
    var generalReducer = function generalReducer() {
        ...
        //
        if (!_startState) _startState = merge({}, state);
        ...
        if (action.isSetState) newState[action.partitionName] = merge({}, state[action.partitionName], action.theirArgs);else if (typeof action.reducer === 'function') newState[action.partitionName] = action.reducer(state[action.partitionName], action);else if (typeof action.pluginCallback === 'function') {
            setTimeout.apply(undefined, [action.pluginCallback, 1].concat(_toConsumableArray(action.theirArgs)));
            return state;
        } else return state; // This is for the redux init   
...
var verifyPlugin = function verifyPlugin(plugin) {
    pluginRequiredKeys.forEach(function (e) {
        ...
        if (typeof plugin.reducer !== 'function' && typeof plugin.pluginCallback !== 'function') error('The plugin must have either a reducer function or a pluginCallback function.');
    });
};
...
function addPlugins(pluginObjs) {
    ...
    pluginObjs = pluginObjs.filter(function (entry) {
        return _typeof(findPlugin(entry.pluginId)) === undefinedString;
    });
...
exports.default = CausalityRedux;
...
if (typeof window !== 'undefined') window['CausalityRedux'] = _causalityRedux2.default;else if (typeof global !== 'undefined') global['CausalityRedux'] = _causalityRedux2.default;
...
var handleAddKeysToProxyObject = exports.handleAddKeysToProxyObject = function handleAddKeysToProxyObject(store, partitionName, currentState, newState) {
    ...
    getKeys(newState[partitionName]).forEach(function (key) {
        if (typeof currentState[partitionName][key] === 'undefined') defineProxyGetSet(proxyObjects[partitionName], store[partitionName], key);
    });
};
